...
連絡帳・メッセージ・メッセージテンプレート・お知らせ・資料・写真・行事・園児情報は保存され、再読み込み後も保持されます。メッセージテンプレートは施設単位で管理し、同じ施設の職員間で共有します。資料と写真は実体を非公開ストレージへ保存し、所属園・クラス・園児の権限確認後に配信します。職員は共有資料の公開を取り消せます。本番環境ではローカルディスクを拒否し、暗号化したS3またはS3互換の非公開バケットを使用します。
